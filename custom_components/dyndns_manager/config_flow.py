...
from .const import (
    ...
    PROVIDER_CLOUDFLARE, PROVIDER_DUCKDNS, PROVIDER_STRATO, PROVIDER_NOIP, PROVIDER_ALLINKL
)

PROVIDER_OPTIONS = [PROVIDER_CLOUDFLARE, PROVIDER_DUCKDNS, PROVIDER_STRATO, PROVIDER_NOIP, PROVIDER_ALLINKL]
...
class DynDNSOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_provider(self, user_input=None):
        ...
        if provider == PROVIDER_DUCKDNS:
            ...
            schema = vol.Schema({
                vol.Required("noip_user", default=self.entry.data.get("noip_user", "")): TextSelector(TextSelectorConfig(type=TextSelectorType.TEXT)),
                vol.Required("noip_pass", default=self.entry.data.get("noip_pass", "")): TextSelector(TextSelectorConfig(type=TextSelectorType.PASSWORD)),
            })
        elif provider == PROVIDER_ALLINKL:
            schema = vol.Schema({
                vol.Required("allinkl_user", default=self.entry.data.get("allinkl_user", "")): TextSelector(TextSelectorConfig(type=TextSelectorType.TEXT)),
                vol.Required("allinkl_pass", default=self.entry.data.get("allinkl_pass", "")): TextSelector(TextSelectorConfig(type=TextSelectorType.PASSWORD)),
            })
        else:
            schema = vol.Schema({
