...
DYNDNS_URL = "https://dynupdate.no-ip.com/nic/update"
...
class NoIPProvider:
    async def update_domain(self, session: aiohttp.ClientSession, credentials: dict, domain: str, ipv4: str | None, ipv6: str | None) -> Tuple[str, str, str, str | None, str | None]:
        user = credentials.get("noip_user")
        pw = credentials.get("noip_pass")
        if not user or not pw:
            raise ValueError("Missing NoIP credentials")

        # No-IP expects one IP per request; prefer explicit values if provided.
        ...
        # Try IPv4 then IPv6 if both provided
        eff_v4 = ipv4
        eff_v6 = ipv6
        status_text = "error"
        dyndns2 = "911"
        ...
        if ipv6:
            ...
            # Prefer latest response; concatenate for visibility
