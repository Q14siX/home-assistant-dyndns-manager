...
class DomainStatusSensor(SensorEntity):
    ...
    def native_value(self):
        ...
        if not st:
            return "INT"
        # Only return DynDNS2 code without IPs
        code = st.get("dyndns2", "INT").split()[0]
        return code or "INT"
    ...
    def extra_state_attributes(self):
        st = self.hass.data[DOMAIN][self.entry.entry_id]["last_results"].get(self._domain, {})
        return {
            ATTR_LAST_UPDATE: st.get("ts"),
            ATTR_IPV4: st.get("ipv4"),
            ATTR_IPV6: st.get("ipv6"),
            ATTR_LAST_RESPONSE: st.get("resp"),
        }